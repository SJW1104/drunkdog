...
서버가 시작되면 백그라운드 작업자가 즉시 한 번, 이후 기본 60초마다 교환 상태를
자동 보정한다. 따라서 사용자 요청이 없어도 D-24 만료, 불가능한 팀 교환 취소,
자동 반복 매칭이 처리된다. 운영 점검을 위해 다음 API로 수동 실행할 수도 있다.
...
실행 간격은 `EXCHANGE_RECONCILE_INTERVAL_SECONDS` 환경변수로 조정한다. `0`으로
설정하면 백그라운드 작업만 중지되며 각 교환 API의 방어적 상태 보정은 유지된다.
...
```json
{
  "original": "학교 수업 만족하나요?",
  "revised": "학교 수업에 얼마나 만족하시나요?",
  "rationale": "질문의 의미는 유지하면서 측정 대상을 더 명확하게 표현했습니다."
}
```

AI는 연구 의도·측정 개념·응답 범위를 새로 만들지 않으며 유도 질문, 이중 질문,
이중부정과 모호한 표현을 줄인다. 이미 적절한 문항은 억지로 바꾸지 않고, 질문 끝에
친근한 문구를 기계적으로 붙이지 않는다. 서버는 모델 출력과 관계없이 `original`을
사용자가 보낸 원문으로 고정하고 수정본과 이유의 길이를 다시 검증한다.

실제 AI를 사용하려면 `AI_MODE=openai`, `OPENAI_API_KEY`, `OPENAI_MODEL`을 설정한다.
기본 모델은 짧고 반복적인 교정 작업에 적합한 `gpt-5.6-luna`이며 Responses API 요청은
구조화 출력과 `store: false`를 사용한다.
