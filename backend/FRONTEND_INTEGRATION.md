...
AI 버튼을 누를 때 프론트에서 문구를 직접 덧붙이거나 원문을 즉시 덮어쓰면 안 된다.
반드시 API가 반환한 `original`, `revised`, `rationale`을 비교 화면에 표시한 뒤 작성자가
원본 유지 또는 수정본 적용을 선택하게 한다. 수정본 적용 후에도 일반 질문 입력창에서
다시 편집할 수 있어야 한다.
